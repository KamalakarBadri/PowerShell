...
def get_file_versions(site_url, list_id, item_id):
    """
    Get ALL versions for a file with selective fields in endpoint
    Using $select to get only needed fields - reduces data transfer
    """
    try:
        # ✅ Filter applied directly in endpoint URL - only select needed fields
...
def process_single_file(site_url, list_id, item, library_title, output_file):
    ...
    try:
        ...
        # Get versions with filters in endpoint
...
def run_performance_tests():
    ...
    print(f"\n📌 Version Endpoint Filter:")
    print(f"  URL: /_api/Web/Lists(guid'{{list_id}}')/items({{item_id}})/versions?$select=VersionId,VersionLabel,Created,IsCurrentVersion,File_x005f_x0020_x005f_Size,OData__x005f_UIVersionString,OData__x005f_CheckinComment&$orderby=Created desc")
    print("="*100)
...
def print_comparison_report(all_results):
    ...
    print("  ✅ Version filter: $select only needed fields reduces data transfer")
...
def main():
    ...
    print(f"\n📌 Version Endpoint Filter (in URL):")
    print(f"  $select=VersionId,VersionLabel,Created,IsCurrentVersion,File_x005f_x0020_x005f_Size,OData__x005f_UIVersionString,OData__x005f_CheckinComment")
    print(f"  $orderby=Created desc")
    print("="*100)
