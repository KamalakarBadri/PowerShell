...
def rotate_csv(site_prefix):
    """Rotate CSV file ONLY when max rows exceeded"""
    global csv_writer, csv_file, current_csv_rows, csv_file_counter
...
def append_to_csv(data, site_prefix):
    """Append row to CSV with rotation ONLY when needed"""
    global csv_writer, csv_file, current_csv_rows
    
    with csv_lock:
        try:
            max_rows = CONFIG['output'].get('csv_max_rows', 300000)
            
            # ONLY rotate if current rows >= max_rows
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                print(f"\n    ⚠️ File {item.get('Id')} timeout after {file_timeout}s")
                return None
...
def process_batch(batch_items, site_url, site_prefix, library_id, library_title, batch_num):
    """Process a batch with timeout to prevent hanging"""
    global STATS, batch_paused
    ...
    batch_timeout = CONFIG['batch_settings'].get('batch_timeout_seconds', 600)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                if processed % 5 == 0:
                    ...
                    print(f"    Processed {processed}/{len(batch_items)} files in {elapsed:.1f}s...", end="\r")
                    
            except TimeoutError:
                print(f"\n    ⚠️ File timeout - continuing...")
                processed += 1
                continue
            except Exception as e:
                error_msg = str(e)
        ...
        print(f"\n    ✅ Batch {batch_num} completed: {len(results)} files")
