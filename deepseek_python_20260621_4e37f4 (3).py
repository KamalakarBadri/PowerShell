...
def update_master_report(current_sites, master_file, config):
    ...
    for site in current_sites:
        ...
        # Build the updated row with safe string handling
        row = {
            'Site ID': site_id,
            ...
            'Owner Email': safe_str(site.get('owner_email', existing_row.get('Owner Email', ''))),
        ...
        if is_first_run:
            ...
            if new_upn and new_upn != old_upn and not is_newly_added:
        ...
        row['User Type'] = safe_str(user_info.get('user_type', existing_row.get('User Type', '')))
        ...
        if is_first_run:
            ...
            # Only track if there's a REAL change (not empty >> empty)
            if new_archive_status != old_archive_status and not is_newly_added:
                # Check if this is a real change (not empty >> empty)
                if old_archive_status or new_archive_status:  # At least one has a value
                    row['Archive Status'] = new_archive_status
        ...
        if user_info.get('exists', False):
            ...
            row['Owner Exists'] = safe_str(existing_row.get('Owner Exists', 'No'))
            row['Owner Status'] = safe_str(user_info.get('status', existing_row.get('Owner Status', 'Not Found')))
    ...
    try:
        ...
        if is_first_run:
            ...
            # Print newly added sites summary
            if newly_added_sites:
                print(f"\n{'='*60}")
                print("🆕 NEWLY ADDED SITES")
                print(f"{'='*60}")
                print(f"Total newly added sites: {len(newly_added_sites)}")
                for site in newly_added_sites[:10]:  # Show first 10
                    print(f"  • {safe_str(site.get('title', 'Unknown'))} (ID: {safe_str(site.get('site_id', 'N/A'))})")
            ...
            # Print changes summary (excluding newly added)
            if all_changes:
                print(f"\n{'='*60}")
