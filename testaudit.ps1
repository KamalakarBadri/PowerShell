...
$ClientSecret = "CyG8Q~FYHut5IejrMc2c24Ziz4a.t"

# Debug mode - SET TO $false FOR PRODUCTION!
$DebugMode = $True  # Shows full tokens when $true

# Calculate date range (always previous month 1st at 04:00:00 UTC to current month 1st at 03:59:59 UTC)
$CurrentDate = [DateTime]::UtcNow
# Get first day of previous month at 04:00:00 UTC
$StartDate = $CurrentDate.AddMonths(-1).Date.AddDays(-$CurrentDate.AddMonths(-1).Day + 1).AddHours(4)
# Get first day of current month at 03:59:59 UTC
$EndDate = $CurrentDate.Date.AddDays(-$CurrentDate.Day + 1).AddHours(3).AddMinutes(59).AddSeconds(59)
...
function Format-AuditData {
    param (
        [object]$AuditData
    )

    # Create ordered dictionary to maintain property order
    $formattedData = [ordered]@{}

    # Add AppAccessContext first with specific properties
    $formattedData['AppAccessContext'] = [ordered]@{
        AADSessionId = $AuditData.AppAccessContext.AADSessionId
        ClientAppName = $AuditData.AppAccessContext.ClientAppName
        CorrelationId = $AuditData.AppAccessContext.CorrelationId
        UniqueTokenId = $AuditData.AppAccessContext.UniqueTokenId
    }

    # Add remaining properties in exact order
    $formattedData['CreationTime'] = $AuditData.CreationTime
    $formattedData['Id'] = $AuditData.Id
    $formattedData['Operation'] = $AuditData.Operation
    $formattedData['OrganizationId'] = $AuditData.OrganizationId
    $formattedData['RecordType'] = $AuditData.RecordType
    $formattedData['UserKey'] = $AuditData.UserKey
    $formattedData['UserType'] = $AuditData.UserType
    $formattedData['Version'] = $AuditData.Version
    $formattedData['Workload'] = $AuditData.Workload
    $formattedData['ClientIP'] = $AuditData.ClientIP
    $formattedData['UserId'] = $AuditData.UserId
    $formattedData['AuthenticationType'] = $AuditData.AuthenticationType
    $formattedData['BrowserName'] = $AuditData.BrowserName
    $formattedData['BrowserVersion'] = $AuditData.BrowserVersion
    $formattedData['CorrelationId'] = $AuditData.CorrelationId
    $formattedData['EventSource'] = $AuditData.EventSource
    $formattedData['GeoLocation'] = $AuditData.GeoLocation
    $formattedData['IsManagedDevice'] = $AuditData.IsManagedDevice
    $formattedData['ItemType'] = $AuditData.ItemType
    $formattedData['ListItemUniqueId'] = $AuditData.ListItemUniqueId
    $formattedData['Platform'] = $AuditData.Platform
    $formattedData['Site'] = $AuditData.Site
    $formattedData['UserAgent'] = $AuditData.UserAgent
    $formattedData['WebId'] = $AuditData.WebId
    $formattedData['DeviceDisplayName'] = $AuditData.DeviceDisplayName
    $formattedData['ApplicationDisplayName'] = $AuditData.ApplicationDisplayName
    $formattedData['ObjectId'] = $AuditData.ObjectId
...
function Save-AuditToCsv {
    ...
    $Report = $Records | ForEach-Object {
        try {
            # Convert auditData to object if it's a string
            $auditData = if ($_.auditData -is [string]) {
                $_.auditData | ConvertFrom-Json
            } else {
                $_.auditData
            }

            # Format the audit data with exact property order
            $formattedAuditData = Format-AuditData -AuditData $auditData
            ...
            Write-Log "Error processing audit data for record $($_.id): $_" -Color "Red"
            continue
        }
    }
