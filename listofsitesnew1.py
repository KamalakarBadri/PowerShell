...
def get_all_sites_from_list(token_manager, sharepoint_admin_url, list_id, page_size=5000):
    ...
    # Using $orderby=Created to ensure consistent ordering across pages
    endpoint = f"{sharepoint_admin_url}/_api/Web/Lists(guid'{list_id}')/items?$skiptoken=0&$top={page_size}&$orderby=Created"
    
    while has_more_pages:
        batch_count += 1
        try:
            print(f"\nProcessing batch {batch_count}...")
            print(f"  Endpoint: {endpoint[:150]}...")  # Truncate for readability
            ...
            if "view threshold" in str(e).lower() or "5000" in str(e) or "throttle" in str(e).lower():
                ...
                print("  Attempting to recover with smaller page size...")
            ...
            # If we have items, break out of the loop
            if all_sites:
                print(f"  Stopping pagination. Retrieved {len(all_sites)} items so far.")
            break
...
def main():
    ...
    try:
        ...
        if sites_after_jan15:
            ...
            print(f"  Latest created date in list: {sorted([s.get('created', '') for s in all_sites if s.get('created')], reverse=True)[:1]}")
