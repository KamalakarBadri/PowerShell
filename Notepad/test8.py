...
<head>
 ...
 .response-content {
            ...
            line-height: 1.4;
            white-space: pre;
            word-break: break-all;  /* Change this to break-all */
            word-wrap: break-word;  /* Add this */
            overflow: auto;         /* Change from overflow-x: auto */
            min-height: 300px;
            max-height: 500px;
            height: 500px;
            width: 100%;
        ...
        /* Additional fix for the response section container */
        .response-section {
            background: white;
            border-radius: 12px;
            overflow: hidden;
            ...
            display: flex;
            flex-direction: column;
            min-width: 0;           /* Add this - important for flex containers */
            max-width: 100%;        /* Add this */
        }
...
<body>
            ...
            <div class="content-area">
                ...
                <div class="response-section">
                    ...
                    <div class="response-body">
                        <div id="response-formatted" class="response-content hljs"><pre><code class="language-json">Response will appear here...</code></pre></div>
                        <div id="response-raw" class="response-content hljs" style="display: none;"><pre><code class="language-json">Raw response will appear here...</code></pre></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function callApi() {
            ...
            formattedResponseDiv.innerHTML = '<pre><code class="language-json">Loading...</code></pre>';
            rawResponseDiv.innerHTML = '<pre><code class="language-json">Loading...</code></pre>';
            ...
            .then(data => {
                if (data.error) {
                    ...
                    formattedResponseDiv.innerHTML = `<pre><code class="language-json">${escapeHtml(errorResponse)}</code></pre>`;
                    rawResponseDiv.innerHTML = `<pre><code class="language-json">${escapeHtml(errorResponse)}</code></pre>`;
                    currentResponse = errorResponse;
                    currentRawResponse = errorResponse;
                } else {
                    setStatus('success', `Success (Status: ${data.status_code})`);
                    
                    // Determine the language class based on content type
                    let langClass = 'language-json';
                    if (data.content_type && (data.content_type.includes('xml') || 
                        (data.response && typeof data.response === 'string' && data.response.trim().startsWith('<')))) {
                    ...
                    formattedResponseDiv.innerHTML = `<pre><code class="${langClass}">${escapeHtml(currentResponse)}</code></pre>`;
                    rawResponseDiv.innerHTML = `<pre><code class="${langClass}">${escapeHtml(currentRawResponse)}</code></pre>`;
            ...
            .catch(err => {
                ...
                formattedResponseDiv.innerHTML = `<pre><code class="language-json">${escapeHtml(errorMsg)}</code></pre>`;
                rawResponseDiv.innerHTML = `<pre><code class="language-json">${escapeHtml(errorMsg)}</code></pre>`;
        ...
        function copyResponse() {
            const responseText = activeTab === 'formatted' ? currentResponse : currentRawResponse;
