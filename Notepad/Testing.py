...
<body>
    ...
    <script>
        ...
        function repairOneDrive() {
            ...
            repairEventSource = new EventSource(`/repair_onedrive_stream?upn=${encodeURIComponent(currentUserUpn)}&onedrive_url=${encodeURIComponent(currentOneDriveUrl)}`);
            
            repairEventSource.onmessage = function(e) {
            ...
            repairEventSource.onerror = function(e) {
...
@app.route('/repair_onedrive_stream')
def repair_onedrive_stream():
    def generate():
        try:
            upn = request.args.get('upn', '').strip()
            onedrive_url = request.args.get('onedrive_url', '').strip()
            
            if not upn or not onedrive_url:
                yield "data: " + json.dumps({"error": "User UPN and OneDrive URL are required", "failed_step": 0}) + "\n\n"
                return
            ...
            if '/personal/' in onedrive_url:
                ...
                yield "data: " + json.dumps({"error": "Invalid OneDrive URL format", "failed_step": 0}) + "\n\n"
                return
            ...
            if not sp_token:
                yield "data: " + json.dumps({"error": "Failed to get SharePoint token", "failed_step": 0}) + "\n\n"
                return
            
            # Get request digest
            request_digest = get_request_digest(site_url, sp_token)
            if not request_digest:
                yield "data: " + json.dumps({"error": "Failed to get request digest", "failed_step": 0}) + "\n\n"
                return
            ...
            try:
                ...
                yield "data: " + json.dumps({"step": 0, "status": "running", "message": "Adding repair account as user"}) + "\n\n"
                ...
                yield "data: " + json.dumps({"step": 0, "status": "success"}) + "\n\n"
                
                # Step 2: Make repair account site admin
                yield "data: " + json.dumps({"step": 1, "status": "running", "message": "Making repair account site admin"}) + "\n\n"
                ...
                yield "data: " + json.dumps({"step": 1, "status": "success"}) + "\n\n"
                
                # Step 3: Get original user ID and remove admin rights
                yield "data: " + json.dumps({"step": 2, "status": "running", "message": "Getting original user and removing admin rights"}) + "\n\n"
                ...
                if original_user_id:
                    if not set_site_admin(site_url, sp_token, request_digest, original_user_id, False):
                        ...
                        yield "data: " + json.dumps({"step": 2, "status": "warning", "message": "Failed to remove original user admin rights, continuing..."}) + "\n\n"
                yield "data: " + json.dumps({"step": 2, "status": "success"}) + "\n\n"
                
                # Step 4: Remove original user from site
                yield "data: " + json.dumps({"step": 3, "status": "running", "message": "Removing original user from site"}) + "\n\n"
                if original_user_id:
                    if not remove_user_by_id(site_url, sp_token, request_digest, original_user_id):
                        ...
                        yield "data: " + json.dumps({"step": 3, "status": "warning", "message": "Failed to remove original user, continuing..."}) + "\n\n"
                yield "data: " + json.dumps({"step": 3, "status": "success"}) + "\n\n"
                
                # Wait a moment for changes to propagate
                time.sleep(2)
                
                # Step 5: Re-add original user to site
                yield "data: " + json.dumps({"step": 4, "status": "running", "message": "Re-adding original user to site"}) + "\n\n"
                new_user_id = ensure_user(site_url, sp_token, request_digest, upn)
                if not new_user_id:
                    raise Exception("Failed to re-add original user to site")
                yield "data: " + json.dumps({"step": 4, "status": "success"}) + "\n\n"
                
                # Step 6: Make original user site admin
                yield "data: " + json.dumps({"step": 5, "status": "running", "message": "Making original user site admin"}) + "\n\n"
                ...
                yield "data: " + json.dumps({"step": 5, "status": "success"}) + "\n\n"
                
                # Step 7: Remove repair account admin rights
                yield "data: " + json.dumps({"step": 6, "status": "running", "message": "Removing repair account admin rights"}) + "\n\n"
                if not set_site_admin(site_url, sp_token, request_digest, repair_user_id, False):
                    ...
                    yield "data: " + json.dumps({"step": 6, "status": "warning", "message": "Failed to remove repair account admin rights, continuing..."}) + "\n\n"
                yield "data: " + json.dumps({"step": 6, "status": "success"}) + "\n\n"
                
                # Step 8: Remove repair account from site
                yield "data: " + json.dumps({"step": 7, "status": "running", "message": "Removing repair account from site"}) + "\n\n"
                if not remove_user_by_id(site_url, sp_token, request_digest, repair_user_id):
                    ...
                    yield "data: " + json.dumps({"step": 7, "status": "warning", "message": "Failed to remove repair account from site"}) + "\n\n"
                yield "data: " + json.dumps({"step": 7, "status": "success"}) + "\n\n"
                
                logger.info(f"OneDrive repair completed successfully for {upn}")
                yield "data: " + json.dumps({"success": True, "message": "OneDrive repair completed successfully"}) + "\n\n"
            ...
            yield "data: " + json.dumps({"error": str(e), "failed_step": 0}) + "\n\n"
