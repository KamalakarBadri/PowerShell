...
class AuditLogCollector:
    ...
    def new_audit_search(self, site, operation):
        if self.active_search_count >= MAX_CONCURRENT_SEARCHES:
            self.log(f"Waiting for available search slot (current: {self.active_search_count}/{MAX_CONCURRENT_SEARCHES})...", Fore.YELLOW)
            return None
        ...
        try:
            ...
            if e.response.status_code in [429, 503]:
                self.log(f"Rate limit hit. Waiting {WAIT_TIME_SECONDS} seconds...", Fore.RED)
    ...
    def save_audit_to_csv(self, records, site, operation):
        ...
        filename = f"{site_name}_{START_DATE.split('T')[0].replace('-', '')}_{END_DATE.split('T')[0].replace('-', '')}_{operation}_{current_time}.csv"
