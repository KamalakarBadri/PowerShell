...
GLOBAL_STATS = {
    ...
    "files_with_more_than_50_versions": 0
}
...
def process_file_item(site_url, list_id, item, library_title):
    ...
    try:
        ...
        if check_versions:
            ...
            GLOBAL_STATS["files_checked"] += 1
            versions = get_file_versions(site_url, list_id, item_id)
            version_count = len(versions)
            GLOBAL_STATS["total_versions_count"] += version_count
            ...
            if versions:
                ...
                # Update global stats
                GLOBAL_STATS["total_versions_size_bytes"] += total_versions_size
                GLOBAL_STATS["total_versions_to_delete_bytes"] += savings['delete_size_bytes']
                GLOBAL_STATS["total_versions_to_keep_bytes"] += savings['keep_size_bytes']
                GLOBAL_STATS["total_versions_to_delete_count"] += savings['delete_count']
                
                if savings['delete_count'] > 0:
                    GLOBAL_STATS["files_with_more_than_50_versions"] += 1
        ...
        for file_item in valid_files:
            ...
            if file_data:
                ...
                if file_data.get('versions_checked', False):
                    print(f" ✓ ({file_data['version_count']} versions, {file_data['total_versions_size_mb']:.2f} MB total)", end="")
                    if file_data.get('versions_to_delete', 0) > 0:
                        print(f" | Will delete {file_data['versions_to_delete']} versions | Save: {file_data['space_saved_gb']:.2f} GB")
                    else:
                        print()
                else:
...
def print_summary_report():
    ...
    print(f"  Files with version check: {GLOBAL_STATS['files_checked']}")
    print(f"  Files with > {CONFIG['keep_last_versions']} versions: {GLOBAL_STATS['files_with_more_than_50_versions']}")
    
    # Size statistics
    print("\n💾 SIZE STATISTICS:")
    current_size_gb = bytes_to_gb(GLOBAL_STATS['total_current_size_bytes'])
    versions_size_gb = bytes_to_gb(GLOBAL_STATS['total_versions_size_bytes'])
    ...
    # Version statistics
    print("\n📄 VERSION STATISTICS:")
    print(f"  Total versions found: {GLOBAL_STATS['total_versions_count']}")
    print(f"  Versions to delete: {GLOBAL_STATS['total_versions_to_delete_count']}")
    print(f"  Versions to keep (last {CONFIG['keep_last_versions']}): {GLOBAL_STATS['total_versions_count'] - GLOBAL_STATS['total_versions_to_delete_count']}")
    
    # Space savings
    print("\n💰 SPACE SAVINGS ANALYSIS:")
    versions_to_delete_gb = bytes_to_gb(GLOBAL_STATS['total_versions_to_delete_bytes'])
    versions_to_keep_gb = bytes_to_gb(GLOBAL_STATS['total_versions_to_keep_bytes'])
    ...
    total_potential_savings_gb = versions_to_delete_gb
    print(f"\n  🎯 If you delete old versions beyond last {CONFIG['keep_last_versions']}:")
    print(f"     You will SAVE approximately: {total_potential_savings_gb:.2f} GB")
    print(f"     This is equivalent to: {total_potential_savings_gb / current_size_gb:.1f}x the current total file size" if current_size_gb > 0 else "")
...
def main():
    ...
    print("(Smart version checking - with Space Savings Analysis)")
    print("="*80)
    ...
    # Print final summary with space savings
