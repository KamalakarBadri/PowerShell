                ...
                foreach ($item in $listItems) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            Write-Host "    Unique permissions check: $hasUniquePerms" -ForegroundColor DarkGray
                        }
                        catch {
                        ...
                        try {
                            ...
                            Write-Host "    Retrieved permissions information successfully" -ForegroundColor DarkGray
                        }
                        catch {
                                ...
                                elseif ($principal.principalType -in @(4,8)) {
                                    ...
                                    try {
                                        ...
                                        if ($members.value.Count -eq 0) {
                                            Write-Host "        No members found in group $groupName - adding group name only" -ForegroundColor Yellow
                                            switch ($role) {
                                        ...
                                        foreach ($member in $members.value) {
                                            ...
                                            if ([string]::IsNullOrEmpty($memberName)) {
                                                Write-Host "            Member name empty, trying to get group title..." -ForegroundColor Yellow
                                                try {
                                                    ...
                                                    Write-Host "            Using group title: $memberName" -ForegroundColor DarkGray
                                                }
                                                catch {
                                                    $memberName = $groupName
                                                    Write-Host "            Using original group name: $memberName" -ForegroundColor DarkGray
                                                }
                                            }
