...
$ClientSecret = "CyG8Q~FYHuCMSyVm4a.t"
...
function Get-AccessToken {
    ...
    try {
        ...
        Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] New access token acquired" -ForegroundColor Green
...
# 3. Check Search Status with simplified error handling
function Get-SearchStatus {
    ...
    try {
        ...
        if ($SearchStatus.status -eq "succeeded" -or $SearchStatus.status -eq "failed") {
            $ActiveSearchCount.Value--
            Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Search $SearchId completed with status: $($SearchStatus.status)" -ForegroundColor Green
        }
        ...
        # Check for token expiration specifically by message content
        if ($errorDetails -and $errorDetails.error -match "expired|InvalidAuthenticationToken") {
            Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Token expired, refreshing..." -ForegroundColor Yellow
            $script:AccessToken = Get-AccessToken
            return Get-SearchStatus -AccessToken $script:AccessToken -SearchId $SearchId -ActiveSearchCount $ActiveSearchCount
        }
        else {
            # For any other error, wait 2 seconds and retry
            Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Error checking status, will retry in 2 seconds: $_" -ForegroundColor Yellow
            Start-Sleep -Seconds 2
            return Get-SearchStatus -AccessToken $AccessToken -SearchId $SearchId -ActiveSearchCount $ActiveSearchCount
        }
    }
}

# 4. Retrieve Records with simplified error handling
    ...
    do {
        try {
            ...
            if ($Response.'@odata.nextLink') {
                Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Retrieved $($AllRecords.Count) records..." -ForegroundColor Yellow
            ...
            if ($errorDetails -and $errorDetails.error -match "expired|InvalidAuthenticationToken") {
                ...
                # For any other error, wait 2 seconds and retry
                Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Error retrieving records, will retry in 2 seconds: $_" -ForegroundColor Yellow
                Start-Sleep -Seconds 2
                continue
            }
        }
