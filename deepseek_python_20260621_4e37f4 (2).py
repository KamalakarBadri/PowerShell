...
def update_master_report(current_sites, master_file, config):
    ...
    for site in current_sites:
        ...
        row = {
            'Site ID': site_id,
            'Site URL': site.get('site_url', existing_row.get('Site URL', '')),
        ...
        # ===== ARCHIVE STATUS =====
        new_archive_status = site.get('archive_status', '').strip()
        old_archive_status = existing_row.get('Archive Status', '').strip()
        ...
        if is_first_run:
            ...
            if newly_added_sites:
                ...
                for site in newly_added_sites[:10]:  # Show first 10
                    print(f"  • {site.get('title', 'Unknown')} (ID: {site.get('site_id', 'N/A')})")
                    print(f"    URL: {site.get('site_url', '')}")
                    print(f"    Owner: {site.get('owner_email', 'Unknown')}")
                    print(f"    Created: {site.get('time_created', 'Unknown')}")
