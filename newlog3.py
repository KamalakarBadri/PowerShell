...
def get_access_token():
    ...
    try:
        ...
        print(f"{Fore.GREEN}[{get_current_time()}] New access token acquired")
        access_token = token_data.get("access_token")
        return access_token
    except requests.exceptions.RequestException as e:
        print(f"{Fore.RED}[{get_current_time()}] Failed to get access token: {e}")
        exit(1)
...
def new_audit_search(site, operation):
    ...
    try:
        ...
        if active_search_count >= MAX_CONCURRENT_SEARCHES:
            print(f"{Fore.YELLOW}[{get_current_time()}] Waiting for available search slot (current: {active_search_count}/{MAX_CONCURRENT_SEARCHES})...")
            return None

        print(f"{Fore.CYAN}[{get_current_time()}] Creating search for {site} - {operation}")
        ...
        print(f"{Fore.GREEN}[{get_current_time()}] Search created (current active: {active_search_count})")
        return search_data.get("id")
    except requests.exceptions.RequestException as e:
        if hasattr(e, 'response') and e.response.status_code in [429, 503]:
            print(f"{Fore.RED}[{get_current_time()}] Rate limit hit when creating search. Waiting {WAIT_TIME_SECONDS} seconds...")
            time.sleep(WAIT_TIME_SECONDS)
            return None
        else:
            print(f"{Fore.RED}[{get_current_time()}] Failed to create audit search for {site} - {operation}: {e}")
            return None
...
def get_search_status(search_id):
    ...
    try:
        ...
        if search_status.get("status") in ["succeeded", "failed"]:
            active_search_count -= 1
            status_color = Fore.GREEN if search_status.get("status") == "succeeded" else Fore.RED
            print(f"{status_color}[{get_current_time()}] Search {search_id} completed with status: {search_status.get('status')}")
        ...
        try:
            ...
            if error_details.get("error", "").lower() in ["expired", "invalidauthenticationtoken"]:
                print(f"{Fore.YELLOW}[{get_current_time()}] Token expired, refreshing...")
        ...
        print(f"{Fore.YELLOW}[{get_current_time()}] Error checking status, will retry in 2 seconds: {e}")
        time.sleep(2)
        return get_search_status(search_id)
    ...
    while url:
        try:
            ...
            if "@odata.nextLink" in data:
                print(f"{Fore.YELLOW}[{get_current_time()}] Retrieved {len(all_records)} records from current query...")
            ...
            # For any other error, wait 2 seconds and retry
            print(f"{Fore.YELLOW}[{get_current_time()}] Error retrieving records, will retry in 2 seconds: {e}")
            time.sleep(2)
            continue
...
def save_audit_to_csv(records, site, operation):
    if not records:
        print(f"{Fore.YELLOW}[{get_current_time()}] No records found for {operation} in {site}")
        return 0
    ...
    try:
        # Always create new file (don't append to existing)
        with open(filename, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=report[0].keys())
            writer.writeheader()
            writer.writerows(report)
        
        print(f"{Fore.GREEN}[{get_current_time()}] Saved {len(records)} records to new file {filename}")
        return len(records)
    except Exception as e:
        print(f"{Fore.RED}[{get_current_time()}] Failed to save CSV: {e}")
        return 0

def generate_summary_file():
    if not summary_data:
        print(f"{Fore.YELLOW}[{get_current_time()}] No data available for summary")
        return
    ...
    try:
        ...
        print(f"{Fore.GREEN}[{get_current_time()}] Summary file generated: {summary_filename}")
    except Exception as e:
        print(f"{Fore.RED}[{get_current_time()}] Failed to generate summary file: {e}")
...
def process_search(key, search_data):
    ...
    while status not in ["succeeded", "failed"]:
        ...
        if status == "succeeded":
            ...
            # Retrieve records
            print(f"{Fore.CYAN}[{get_current_time()}] Retrieving records for {site} - {operation}")
            ...
            print(f"{Fore.RED}[{get_current_time()}] Search {search_id} failed")
            completed_searches[key] = {
                "CompletedTime": datetime.now().isoformat() + "Z",
                "Status": "failed"
            }
            ...
            print(f"{Fore.YELLOW}[{get_current_time()}] Search {search_id} status: {status}. Waiting {RETRY_DELAY_SECONDS} seconds...")
            time.sleep(RETRY_DELAY_SECONDS)

def main():
    global access_token, existing_searches, completed_searches, active_search_count, summary_data
    
    print(f"{Fore.CYAN}[{get_current_time()}] Starting audit log collection process...")

    # Step 1: Authenticate
    print(f"{Fore.YELLOW}[{get_current_time()}] Authenticating...")
    access_token = get_access_token()

    # Step 2: Load existing searches from tracking files
    existing_searches = load_searches_from_file("search_ids.json")
    print(f"{Fore.YELLOW}[{get_current_time()}] Loaded {len(existing_searches)} existing searches from file.")
    ...
    with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_SEARCHES) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                print(f"{Fore.RED}[{get_current_time()}] Error processing search: {e}")
    ...
    if len(completed_searches) == total_searches:
        print(f"{Fore.CYAN}[{get_current_time()}] All operations completed successfully!")
        ...
        print(f"{Fore.YELLOW}[{get_current_time()}] {remaining} searches remaining. Run the script again to continue.")
