...
# New NameId cache - same for entire session
NEW_NAMEID_CACHE = {
    'nameid': None,
    'target_upn': None
}
...
def get_cached_new_nameid(target_upn):
    """Get cached new NameId or fetch new one if not available"""
    # Check if we already have the new NameId for this target user
    if (NEW_NAMEID_CACHE['nameid'] is not None and 
        NEW_NAMEID_CACHE['target_upn'] == target_upn):
        logger.info(f"Using cached new NameId for {target_upn}")
        return NEW_NAMEID_CACHE['nameid']
    
    # New NameId not cached or for different user, fetch it
    logger.info(f"Fetching new NameId for {target_upn}")
    new_nameid = get_new_site_nameid(target_upn)
    
    # Update cache
    NEW_NAMEID_CACHE['nameid'] = new_nameid
    NEW_NAMEID_CACHE['target_upn'] = target_upn
...
def process_site(target_upn, site_identifier, site_type, remove_mismatch=False, new_nameid=None):
    ...
    try:
        ...
        if user_info:
            ...
            # Use the pre-fetched new NameId for comparison
...
def main():
    ...
    try:
        ...
        # Pre-fetch tokens and new NameId to warm up the cache
        logger.info("Pre-fetching access tokens and new NameId...")
        get_cached_token('sharepoint')
        if args.type == 'onedrive':
            get_cached_token('graph')
        
        # Get the new NameId once for the entire session
        new_nameid = get_cached_new_nameid(args.target)
        if new_nameid:
            logger.info(f"Using new NameId for session: {new_nameid}")
        else:
            logger.warning("Could not retrieve new NameId from reference site")
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            # Submit all tasks with the pre-fetched new NameId
            future_to_site = {
                executor.submit(process_site, args.target, site, args.type, remove_mismatch=False, new_nameid=new_nameid): site 
                for site in sites
            }
